...
import { publicProcedure, protectedProcedure, adminProcedure, router } from "./trpc.ts";
...
export const appRouter = router({
  // Login is now handled by Authentik OAuth2
  ...
  generateNewAvatar: protectedProcedure.query(async () => {
  ...
  getActiveMonth: protectedProcedure.query(async () => {
  ...
  getFocusedTaskIds: protectedProcedure.query(async () => {
    const tasks = await db.query.task.findMany({
      where: and(isNotNull(schema.task.monthId), eq(schema.task.isFocused, 1)),
    });
    return tasks.map((task) => task.id);
  }),
  getCurrentMonthTasks: protectedProcedure.query(async () => {
    const currentMonth = await db.query.month.findFirst({
      where: eq(schema.month.isActive, 1),
    });
    if (!currentMonth) {
  ...
  getTemplateTasks: protectedProcedure.query(async () => {
  ...
  getTemplateTask: protectedProcedure
  ...
  getBacklogTasks: protectedProcedure.query(async () => {
  ...
  getTasksByCategoryId: protectedProcedure
    .input(type({ categoryId: "string" }))
    .query(async ({ input }) => {
  ...
  onClearCache: protectedProcedure.subscription(() => {
